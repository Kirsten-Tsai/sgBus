...
class BusStop {
  ...
  public Set<BusService> getBusServices() {
    ...
    try {
      ...
      this.name = s.nextLine(); //this bus stop name
      String line = s.nextLine(); // all the available buses
      String[] busIds = line.split(",");
      Set<BusService> tmpBusServices = new HashSet<>();
      for (String id : busIds) {
        tmpBusServices.add(new BusService(id)); //add all the available buses
      }
      return tmpBusServices;    //Set<BusService> 
    } catch (IOException e) {
